...
async function seedGoldenJordanTouringRoutes(prisma: PrismaService) {
  const canonicalByReviewKey = new Map(GOLDEN_JORDAN_TOURING_ROUTES.map((route) => [normalizeTouringRouteReviewKey(route.name), route]));
  const records = [];
  ...
  for (const route of GOLDEN_JORDAN_TOURING_ROUTES) {
    ...
    const routeData = {
      ...
      reviewNotes: 'Golden Jordan canonical touring route. Operational infrastructure only; not a sellable excursion template.',
      active: true,
    };

    records.push(
      await (prisma as any).touringRoute.upsert({
  ...
  const existingRoutes = await (prisma as any).touringRoute.findMany({
    where: { code: { notIn: GOLDEN_JORDAN_TOURING_ROUTES.map((route) => route.code) } },
    select: { id: true, code: true, name: true, routeDescription: true },
  });

  for (const existing of existingRoutes) {
    const matched = canonicalByReviewKey.get(normalizeTouringRouteReviewKey(existing.name || existing.routeDescription || ''));
    if (!matched) {
      continue;
    }

    await (prisma as any).touringRoute.update({
      where: { id: existing.id },
      data: {
        reviewNotes: `Review duplicate/similar legacy touring route against canonical ${matched.code}. Do not delete automatically.`,
      },
    });
  }

// ===========================================================================
// Phase P.3X-5E-3A — API-LOCAL COPY of the admin-web R.7B narrative helper.
//
// This is a VERBATIM, byte-for-byte copy of
//   apps/admin-web/app/quotes/[id]/day-narrative-preview.ts
// brought into the API so proposal-v3 can eventually reach the deterministic
// multilingual (en/es/pt/ar) day-narrative renderer. The repo has no shared
// package today (workspaces = apps/*), so the safe zero-behavior slice is to
// duplicate the PURE helper here and lock both copies with a golden parity test
// (proposal-v3-narrative-parity.test.ts) until a shared package is introduced.
//
// IMPORTANT — this copy is DORMANT:
//   • proposal-v3 does NOT import or call it yet (P.3X-5E-3A is copy + tests only).
//   • Automatic replacement of existing day.notes is BLOCKED until a
//     notesLanguage / provenance decision is made (P.3X-5E-3B will only ever
//     generate fallback narrative for EMPTY/placeholder days, never override prose).
//   • Keep this byte-identical to the admin-web original; the parity test fails
//     on any output drift. Do not edit logic here independently.
// ===========================================================================

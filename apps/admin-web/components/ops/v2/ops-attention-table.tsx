        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground">
              <th scope="col" className="whitespace-nowrap px-3 py-2 font-medium">Booking</th>
              <th scope="col" className="whitespace-nowrap px-3 py-2 font-medium">Client</th>
              <th scope="col" className="whitespace-nowrap px-3 py-2 font-medium">Trip</th>
              <th scope="col" className="whitespace-nowrap px-3 py-2 font-medium">Status</th>
              <th scope="col" className="whitespace-nowrap px-3 py-2 font-medium">Blockers</th>
              <th scope="col" className="whitespace-nowrap px-3 py-2 font-medium">Finance</th>
              <th scope="col" className="whitespace-nowrap px-3 py-2 text-right font-medium">Actions</th>
            </tr>
          </thead>
          <tbody>
            {queue.rows.map((row) => (
              <tr key={row.id} className="border-b border-border align-top last:border-0">
                <td className="px-3 py-2">
                  <p className="whitespace-nowrap font-medium text-foreground">{row.bookingRef}</p>
                ...
                <td className="px-3 py-2 text-muted-foreground">
                  <p className="whitespace-nowrap text-foreground">{row.dateLabel ?? '—'}</p>
                  <p className="whitespace-nowrap text-xs">{row.pax != null ? `${row.pax} pax` : '—'}</p>
                </td>
                <td className="px-3 py-2">
                  <OperationalStatusBadge variant={row.statusVariant} label={row.status} />
                </td>
                <td className="px-3 py-2">
                ...
                <td className="px-3 py-2">
                  <div className="flex flex-col items-start gap-1">
                    <OperationalStatusBadge variant={row.invoiceVariant} label={row.invoiceStatus} prefix="Invoice" />
                    <OperationalStatusBadge variant={row.supplierVariant} label={row.supplierPaymentStatus} prefix="Supplier" />
                  </div>
                </td>
                <td className="px-3 py-2">
                  <div className="flex flex-col items-end gap-1 whitespace-nowrap text-right">
                    <a href={`/operations/v2/${row.id}`} className="text-xs font-medium text-primary hover:underline">
                      View in V2
                    </a>
                    <a href={`/bookings/${row.id}/operations`} className="text-xs text-muted-foreground hover:underline">
                      Open in Classic
                    </a>
                  </div>
                </td>

...
describe('Phase S.2D-2 — Route Planner "Apply to Day" (existing day PATCH only)', () => {
  ...
  it('S.2D-3B: Classic Jordan Route Template preview (duration-driven, preview-only)', () => {
    expectSourceContains(source, [
      ...
      'Preview only — templates are not saved yet.',
      ...
      // Apply button is present but DISABLED (apply lands in S.2D-3C)
      'Template apply comes in the next phase.',
    ]);
    // The template preview itself performs NO write — only the disabled Apply
    // placeholder. (Network safety for the whole component is asserted below.)
    assert.ok(/disabled title="Template apply comes in the next phase\."/.test(source), 'Apply Template button is disabled');
    // The outdated S.2D-1 warning copy is gone (the 5-day template now exists).

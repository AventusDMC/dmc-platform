...
import { buildQuoteReadinessModel, buildQuoteWorkspaceHref, classicQuoteBasePath, getQuoteServiceCategoryKey, isActiveImportedQuoteServiceUnresolved, type QuotePricingFocus, type ServicePlannerCategory } from './quote-readiness';
...
export type QuoteDetailsPageProps = {
  params: Promise<{
    id: string;
  }>;
...
export async function ClassicQuoteWorkspace({ params, searchParams }: QuoteDetailsPageProps) {
          ...
          <QuoteBuilderEmptyState
            ...
            action={
              <div className="table-action-row">
                <Link href={classicQuoteBasePath(id)} className="primary-button">
                  Retry
                </Link>
  ...
  const buildTabHref = (tab: QuoteDetailTab) => `${classicQuoteBasePath(quote.id)}?tab=${tab}`;
        ...
        <div className="quote-builder-shell">
          ...
          {activeTab === 'itinerary' && activeStep === 'journey-composer' ? (
            ...
            <GuidedJourneyComposer
              quote={quote as any}
              advancedWorkspaceUrl={`${classicQuoteBasePath(quote.id)}?tab=itinerary&step=itinerary`}
            />
          ) : null}

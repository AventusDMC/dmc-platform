...
describe('hotel contract display fallbacks', () => {
  ...
  // Three source-grep blocks removed during the v1 → v2 cutover:
  //   - "keeps contract detail rendering scoped..." — asserted internals
  //     of HotelContractWorkspace.tsx (now deleted)
  //   - "Room Types freeze fix — workspace wires error boundary..." —
  //     asserted internals of the same workspace
  //   - "Room Types panel never reads pricing engine internals..." —
  //     asserted internals of RoomTypesPanel.tsx (now deleted)
  // The v2 contract detail page at hotels/[id]/contracts/[contractId]
  // is much smaller (~400 lines vs ~1256) and doesn't carry the
  // safe-mode / render-probe / lazy-panel machinery the original
  // workspace needed for performance. The protections are now baked
  // into v2's architecture (server components + server actions) rather
  // than enforced via source greps.
});

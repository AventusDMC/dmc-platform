...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string; departureId: string }> }) {
  ...
  try {
    return proxyRequest(request, `${API_BASE_URL}/series/${id}/departures/${departureId}/regenerate-services`, 'POST');
  } catch (error) {
    return proxyFetchErrorResponse(error, 'Could not regenerate series services.');
  }
}

...
export async function POST(
  ...
  try {
    response = await fetch(`${API_BASE_URL}/quotes/${id}/options/${optionId}/items`, {
      method: 'POST',
      headers: {
    ...
    return proxyFetchErrorResponse(error, 'Could not reach API server while saving quote option item.');
  }

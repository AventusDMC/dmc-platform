...
async function getHotel(id: string): Promise<HotelHeader | null> {
  try {
    ...
    console.error('[hotels/rooms] hotel header unavailable', error);
    return null;
  }
}
...
async function getRooms(hotelId: string): Promise<RoomCategorySummary[]> {
  try {
    ...
    console.error('[hotels/rooms] rooms list unavailable', error);
    return [];
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine — Room types</p>
        ...
        {/* Existing rooms */}
        <section data-testid="hotel-room-list">
          <div
            style={{
              display: 'flex',
              flexWrap: 'wrap',
                      ...
                      <tr key={room.id}>
                        ...
                        <td>
                          <div className="table-action-row" style={{ display: 'flex', gap: '0.4rem' }}>
                            <Link
                              href={`/hotels/${hotelId}/rooms/${room.id}/edit`}
                              prefetch={false}
                              className="compact-button"
                            >
                              Edit

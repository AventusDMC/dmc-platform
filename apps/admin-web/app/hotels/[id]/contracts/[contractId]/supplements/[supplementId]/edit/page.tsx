...
// Hotels Engine — supplement edit (PR-A6).
//
...
async function getContract(contractId: string): Promise<ContractSummary | null> {
  try {
    return await adminPageFetchJson<ContractSummary>(
      ...
      'Hotel supplement edit — contract',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels/supplement/edit] contract unavailable', error);
    return null;
  }
}
...
async function getSupplement(
  ...
  try {
    ...
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels/supplement/edit] supplement unavailable', error);
    return null;
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine — Edit supplement</p>
        ...
        <section
          data-testid="hotel-supplement-edit"

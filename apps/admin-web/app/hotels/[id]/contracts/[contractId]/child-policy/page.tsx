...
async function getContract(contractId: string): Promise<ContractSummary | null> {
  try {
    return await adminPageFetchJson<ContractSummary>(
      ...
      'Hotel child policy — contract',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels/contract/child-policy] contract unavailable', error);
    return null;
  }
}
...
async function getPolicy(contractId: string): Promise<ChildPolicy | null> {
  try {
    return await adminPageFetchJson<ChildPolicy | null>(
      ...
      'Hotel child policy — policy',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels/contract/child-policy] policy unavailable', error);
    return null;
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine — Child policy</p>
        ...
        <section
          data-testid="hotel-child-policy-header"
        ...
        {/* Add band form */}
        <section
          data-testid="hotel-child-policy-band-create"
          className="detail-card"
          style={{ marginBottom: '1.5rem' }}
        >
          <h2 className="section-title" style={{ fontSize: '1.05rem', marginBottom: '0.6rem' }}>
        ...
        {/* Bands list */}
        <section data-testid="hotel-child-policy-band-list">
          <h2 className="section-title" style={{ fontSize: '1.05rem', marginBottom: '0.6rem' }}>

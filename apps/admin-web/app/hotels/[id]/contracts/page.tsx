...
// Hotels Engine — contracts list for a single hotel (PR-A1).
//
...
async function getHotel(id: string): Promise<HotelHeader | null> {
  try {
    return await adminPageFetchJson<HotelHeader>(
      ...
      'Hotel contracts header',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels/contracts] hotel header unavailable', error);
    return null;
  }
}
...
async function getContracts(hotelId: string): Promise<ContractListItem[]> {
  try {
    ...
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels/contracts] contracts list unavailable', error);
    return [];
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine — Contracts</p>
        ...
        {/* Create form */}
        <section
          data-testid="hotel-contract-create"
        ...
        {/* List */}
        <section data-testid="hotel-contract-list">
          <h2 className="section-title" style={{ fontSize: '1.05rem', marginBottom: '0.6rem' }}>
                  ...
                  {contracts.map((c) => {
                    ...
                    return (
                      <tr key={c.id}>
                        ...
                        <td>
                          <Link
                            href={`/hotels/${hotelId}/contracts/${c.id}`}
                            prefetch={false}

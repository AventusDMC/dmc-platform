...
async function getRoom(roomId: string): Promise<RoomCategoryDetail | null> {
  try {
    ...
    console.error('[hotels-v2/rooms/edit] room detail unavailable', error);
    return null;
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine v2 — Edit room type</p>
          ...
          <p className="table-subcopy" style={{ marginTop: '0.5rem' }}>
            <Link href={`/hotels-v2/${hotelId}/rooms`} prefetch={false}>
              ← Back to room types
            </Link>
          </p>
        </header>

...
// Hotels Engine v2 — contract meal plans (PR-A9 — final wizard slice).
//
...
async function getContract(contractId: string): Promise<ContractSummary | null> {
  try {
    return await adminPageFetchJson<ContractSummary>(
      ...
      'Hotel v2 meal plans — contract',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels-v2/contract/meal-plans] contract unavailable', error);
    return null;
  }
}
...
async function getMealPlans(contractId: string): Promise<MealPlanRow[]> {
  try {
    const rows = await adminPageFetchJson<MealPlanRow[]>(
      ...
      'Hotel v2 meal plans — list',
      { cache: 'no-store' },
    );
    return Array.isArray(rows) ? rows : [];
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels-v2/contract/meal-plans] meal plans unavailable', error);
    return [];
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine v2 — Meal plans</p>
        ...
        {/* Create form */}
        <section
          data-testid="hotel-v2-meal-plan-create"
        ...
        {/* List */}
        <section data-testid="hotel-v2-meal-plan-list">
          <h2 className="section-title" style={{ fontSize: '1.05rem', marginBottom: '0.6rem' }}>
        ...
        {/* Stop-gap to legacy for audit history + default toggle */}
        <section
          data-testid="hotel-v2-meal-plan-legacy-bridge"
          className="detail-card"
          style={{
            marginTop: '1.5rem',
            marginBottom: '1rem',
        ...
        >
          <h2 className="section-title" style={{ fontSize: '1rem', marginBottom: '0.4rem' }}>
            Marking a meal plan as default + audit history
          </h2>
          <p className="table-subcopy" style={{ margin: 0 }}>
            Add / toggle active / delete + notes edit all work inline here. Marking a meal
            plan as the contract default (the one the quote engine picks when none is
            specified) and the per-row audit log still live in the legacy workspace:{' '}
            <Link
        ...
        >
          /hotels-v2/{hotelId}/contracts/{contractId}/meal-plans — server-rendered. Create

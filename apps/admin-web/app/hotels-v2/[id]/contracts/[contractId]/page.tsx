...
// Hotels Engine v2 — single contract detail (PR-A1).
//
...
async function getContractSummary(contractId: string): Promise<ContractSummary | null> {
  try {
    return await adminPageFetchJson<ContractSummary>(
      ...
      'Hotel v2 contract summary',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels-v2/contract] summary unavailable', error);
    return null;
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine v2 — Contract</p>
        ...
        {/* Master data */}
        <section
          data-testid="hotel-v2-contract-master"
        ...
        {/* Counts row */}
        <section
          data-testid="hotel-v2-contract-counts"
          style={{
            display: 'grid',
        ...
        {/* Per-room aggregate */}
        <section data-testid="hotel-v2-contract-rooms" style={{ marginBottom: '1.5rem' }}>
          <h2 className="section-title" style={{ fontSize: '1.05rem', marginBottom: '0.6rem' }}>
        ...
        >
          /hotels-v2/{hotelId}/contracts/{contract.id} — read-only summary. v2 editors arrive
          in subsequent PRs.
        </p>
      </section>
    </main>

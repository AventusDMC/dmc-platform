...
// Hotels Engine v2 — contract supplements list + inline create (PR-A5).
//
...
async function getContract(contractId: string): Promise<ContractSummary | null> {
  try {
    return await adminPageFetchJson<ContractSummary>(
      ...
      'Hotel v2 supplements — contract',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels-v2/contract/supplements] contract unavailable', error);
    return null;
  }
}
...
async function getSupplements(contractId: string): Promise<SupplementRow[]> {
  try {
    const rows = await adminPageFetchJson<SupplementRow[]>(
      ...
      'Hotel v2 supplements — list',
      { cache: 'no-store' },
    );
    return Array.isArray(rows) ? rows : [];
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels-v2/contract/supplements] supplements unavailable', error);
    return [];
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine v2 — Supplements</p>
        ...
        {/* Create form */}
        <section
          data-testid="hotel-v2-supplement-create"
        ...
        <section data-testid="hotel-v2-supplement-list">
          <h2 className="section-title" style={{ fontSize: '1.05rem', marginBottom: '0.6rem' }}>
          ...
          {supplements.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              {groups.map((g) => (
                <div key={g.type} data-testid="hotel-v2-supplement-type-group">
                  <h3

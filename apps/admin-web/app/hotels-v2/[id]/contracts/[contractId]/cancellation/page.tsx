...
// Hotels Engine v2 — cancellation policy editor (PR-A7).
//
...
async function getContract(contractId: string): Promise<ContractSummary | null> {
  try {
    return await adminPageFetchJson<ContractSummary>(
      ...
      'Hotel v2 cancellation — contract',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels-v2/contract/cancellation] contract unavailable', error);
    return null;
  }
}
...
async function getPolicy(contractId: string): Promise<CancellationPolicy | null> {
  try {
    return await adminPageFetchJson<CancellationPolicy | null>(
      ...
      'Hotel v2 cancellation — policy',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels-v2/contract/cancellation] policy unavailable', error);
    return null;
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine v2 — Cancellation policy</p>
        ...
        <section
          data-testid="hotel-v2-cancellation-policy-header"
        ...
        {/* Add rule form */}
        <section
          data-testid="hotel-v2-cancellation-rule-create"
          className="detail-card"
          style={{ marginBottom: '1.5rem' }}
        >
          <h2 className="section-title" style={{ fontSize: '1.05rem', marginBottom: '0.6rem' }}>
        ...
        {/* Rules list */}
        <section data-testid="hotel-v2-cancellation-rule-list">
          <h2 className="section-title" style={{ fontSize: '1.05rem', marginBottom: '0.6rem' }}>
        ...
        {/* Stop-gap to legacy for rule edit + audit history */}
        <section
          data-testid="hotel-v2-cancellation-legacy-bridge"
          className="detail-card"
          style={{
            marginTop: '1.5rem',
            marginBottom: '1rem',
        ...
        >
          <h2 className="section-title" style={{ fontSize: '1rem', marginBottom: '0.4rem' }}>
            Editing existing rules + audit history
          </h2>
          <p className="table-subcopy" style={{ margin: 0 }}>
            Policy header + add / delete rules work inline here. Single-rule edit (change
            window / penalty without recreating) and the per-rule audit log still live in the
            legacy workspace:{' '}
            <Link
        ...
        >
          /hotels-v2/{hotelId}/contracts/{contractId}/cancellation — server-rendered.

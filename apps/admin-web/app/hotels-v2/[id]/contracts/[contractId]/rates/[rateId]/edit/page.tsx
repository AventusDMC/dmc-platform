...
// Hotels Engine v2 — rate cell edit (PR-A4).
//
...
async function getContract(contractId: string): Promise<ContractSummary | null> {
  try {
    return await adminPageFetchJson<ContractSummary>(
      ...
      'Hotel v2 rate edit — contract',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels-v2/rate/edit] contract unavailable', error);
    return null;
  }
}
...
async function getRate(rateId: string): Promise<RateRow | null> {
  try {
    return await adminPageFetchJson<RateRow>(
      ...
      'Hotel v2 rate edit — rate',
      { cache: 'no-store', allow404: true },
    );
  } catch (error) {
    if (isNextRedirectError(error)) throw error;
    console.error('[hotels-v2/rate/edit] rate unavailable', error);
    return null;
  }
}
    ...
    <main className="page">
      <section className="panel workspace-panel workspace-panel-wide">
        <header style={{ marginBottom: '1.5rem' }}>
          <p className="eyebrow">Hotels Engine v2 — Edit rate cell</p>
        ...
        <section
          data-testid="hotel-v2-rate-edit"
        ...
        <section
          data-testid="hotel-v2-rate-delete"
          className="detail-card"
        ...
        >
          /hotels-v2/{hotelId}/contracts/{contractId}/rates/{rateId}/edit — server-rendered.

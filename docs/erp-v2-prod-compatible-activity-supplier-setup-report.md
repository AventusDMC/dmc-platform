# ERP V2 — Production Compatible Synthetic Activity Supplier Setup Report

**Date:** 2026-07-17
**Status:** Production setup execution. One compatible synthetic **activity** supplier created to unblock the
Packet V2 test-record setup; no assignment, no confirmation, no flags, no send. No code, schema, flag/env,
or additional production/staging change accompanies this report.

## 1. Environment
- **Production.**
- **Compatible synthetic activity supplier setup only.**
...
## 3. Created supplier
- Label: **`ZZZ TEST ACTIVITY SUPPLIER — DO NOT SEND`**.
- Type: **`activity`**.
- Email: **`zzz-test-activity-supplier@axis.invalid`**.
...
## 6. Net conclusion
- The **compatible synthetic activity supplier now exists in production**.
- It **should satisfy the ACTIVITY-row compatibility rule** (type `activity` matches the
  `ACTIVITY / EXCURSION / EXPERIENCE / TOUR / ATTRACTION` requirement).
- **`BK-2026-0006` Activity-service assign / confirm remains a separate approved step.**
...
- No secrets, passwords, DB URLs, credentials, internal hosts, raw deployment URLs, project identifiers,
  ...
  dependency counts, flag / role names, and results.
